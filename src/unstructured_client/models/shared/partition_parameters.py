...
class PartitionParametersTypedDict(TypedDict):
    ...
    skip_table_chunking: NotRequired[bool]
    r"""When `True`, tables are not chunked and always kept unchanged. Default: False"""
...
class PartitionParameters(BaseModel):
    ...
    skip_table_chunking: Annotated[Optional[bool], FieldMetadata(multipart=True)] = (
        False
    )
    r"""When `True`, tables are not chunked and always kept unchanged. Default: False"""
    ...
    @field_serializer("output_format")
    def serialize_output_format(self, value):
        if isinstance(value, str):
            try:
                return shared.OutputFormat(value)
            except ValueError:
                return value
        return value
    ...
    def serialize_strategy(self, value):
        if isinstance(value, str):
            try:
                return shared.Strategy(value)
            except ValueError:
                return value
        return value

    @field_serializer("vlm_model_provider")
    def serialize_vlm_model_provider(self, value):
        if isinstance(value, str):
            try:
                return shared.VLMModelProvider(value)
            except ValueError:
                return value
        return value
